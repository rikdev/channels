...
struct is_executor<Executor, decltype(execute(std::declval<Executor>(), std::declval<void()>()))> : std::true_type {};
...
{
	static_assert(is_executor_v<Executor>, "You must make a function with a prototype 'execute(Executor, void())'");
#if __cpp_lib_is_invocable
	...
	private:
		void invoke(const shared_value_type& shared_value) override
		{
			...
			auto task = [self = this->shared_from_this(), value = shared_value]() mutable
			{
				...
				compatibility::apply(local_self->callback_, *local_value);
			};
